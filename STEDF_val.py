...
# --- 配置参数 ---
model_path = "./output/SDv1.5_fulltimesteps_2upblockentire_train"
dataset_path = "../datasets/COCO-Caption"
device = "cuda:0"
trigger = "Trigger:"
save_path = "contrastive_model_mutilmodels.pth"
num_samples_to_evaluate = 1000  # 只需要评估1000个样本
...
if __name__ == "__main__":
    ...
    # 初始化计数器
    tp_count = 0  # True Positive: 预测为后门 (1), 实际为后门 (1)
    tn_count = 0  # True Negative: 预测为良性 (0), 实际为良性 (0)
    fp_count = 0  # False Positive: 预测为后门 (1), 实际为良性 (0)
    fn_count = 0  # False Negative: 预测为良性 (0), 实际为后门 (1)
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(min(num_samples_to_evaluate, len(dataset))), desc="Evaluating"):
            ...
            # 运行分类器
            logits = model(inputs)  # logits是一个 [2, 1] 的张量
            # print(logits)
            # 根据 logits 预测结果。>0 预测为后门(1)，<0 预测为良性(0)
